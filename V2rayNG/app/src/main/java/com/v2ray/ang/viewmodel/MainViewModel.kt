...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val realPingFinishedAction by lazy { MutableLiveData<Boolean>() }
    ...
    fun selectFastestAvailableServer(): String? {
        val best = serversCache
            .mapNotNull { item ->
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_MEASURE_CONFIG_FINISH -> {
                    ...
                    if (content == "0") {
                        onTestsFinished()
                        realPingFinishedAction.value = true
                    }
                }
            }
        }
